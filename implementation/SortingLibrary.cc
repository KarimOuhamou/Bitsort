...
Versione di Partenza che implementa array di frequenze e pair di Bit per detectare i doppioni e saltare alcune letture
Ancora piu efficiente in termini di letture ma troppo overhead (Array da 10k elementi)

int* Bitsort(std::ifstream& input_data, const int datasetSize){
    int* A = new int[datasetSize];
    unsigned int bitmask[632]={0};
    unsigned int frequency[10250]={0};

    //Popolamento bitmask e frequency
    for (int i = 0; i < datasetSize; i++) {
        ct_cmp++;
        ...
        offset<<= (value %16)*2;
        unsigned int &chunk = bitmask[value/16];
        ct_read++;
